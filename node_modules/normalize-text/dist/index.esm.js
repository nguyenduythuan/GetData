...
var normalizeName = function (name, exceptions) {
	...
	return splitWords(name).map(function (word) { return exceptions.indexOf(word) > -1 ? word : capitalizeFirstLetter(word); }).join(' ');
};
...
var normalizeParagraph = function (text) {
  var whiteSpacesNormalizedText = normalizeWhiteSpaces(text);
  return capitalizeFirstLetter(getLastChar(whiteSpacesNormalizedText) === '.' ? whiteSpacesNormalizedText : whiteSpacesNormalizedText + '.');
};
...
var normalizeText = function (texts) { return normalizeWhiteSpaces(normalizeDiacritics(resolveToText(texts))).toLocaleLowerCase(); };

export default normalizeText;
export { capitalizeFirstLetter, normalizeDiacritics, normalizeName, normalizeParagraph, normalizeText, normalizeWhiteSpaces };
